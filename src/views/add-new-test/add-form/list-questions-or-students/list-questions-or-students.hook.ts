...
const useListQuestionsOrStudents = (areQuestions: boolean, toggleChosenIds: (id: number, isAdd: boolean) => void) => {
    ...
    if (areQuestions) {
      ...
      .then(async response => {
        if (response.ok) {
          ...
          // Set available ids
          setAvailableIds(data.content.map((question: any) => question.id));
      ...
      .then(async response => {
        if (response.ok) {
          ...
          // Set available ids
          setAvailableIds(filteredStudents.map((user: any) => user.id));
